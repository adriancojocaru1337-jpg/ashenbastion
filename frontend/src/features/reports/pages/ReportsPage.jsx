...
function renderPayload(report) {
  ...
  return <div className="grid" style={{gap:8}}>
    ...
    <div className="card">Target: {payload.target?.name} • {payload.target?.type}</div>
    <div className="card">Loot: Timber {payload.loot?.timber || 0}, Iron {payload.loot?.iron || 0}, Grain {payload.loot?.grain || 0}, Ember {payload.loot?.ember || 0}</div>
    <div className="card">Survivors: {(payload.survivors || []).map(s => `${s.troop_type} x${s.quantity_survived}`).join(', ') || 'None'}</div>
  </div>;
}
...
export default function ReportsPage() {
  ...
  return <div className="grid" style={{gap:16}}>
    <SectionPanel title="Reports" description="Scouting and raid results appear here after marches resolve.">
